...
async function snapshot(client) {
  ...
  const t = await client.query('SELECT * FROM todos WHERE plan_id=$1 AND deleted_at IS NULL ORDER BY created_at ASC', [currentPlan()]);
  const d = await client.query(`SELECT d.*, t.text AS todo_text FROM do_records d JOIN todos t ON t.todo_id=d.todo_id WHERE d.plan_id=$1 AND t.plan_id=$1 AND t.deleted_at IS NULL ORDER BY d.created_at DESC`, [currentPlan()]);
  const reviewDate = kstDate();
  const see = await syncSeeRecord(client, currentPlan(), reviewDate);
  const adjQ = await client.query(`SELECT s.*, p.title AS target_title, p.start_date AS target_start_date, p.end_date AS target_end_date FROM see_adjustments s JOIN plans p ON p.plan_id=s.target_plan_id WHERE s.source_plan_id=$1 LIMIT 1`, [currentPlan()]);
...
async function saveDo(client, body) {
  ...
  const todo = await client.query('SELECT todo_id FROM todos WHERE todo_id=$1 AND plan_id=$2 AND deleted_at IS NULL', [todoId, currentPlan()]);
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if(action==='scope-status' && req.method==='GET'){
      const selected=await selectedScope(client,req);
      return json(res,200,{selected:selected?.scope||null,scope:selected?{label:selected.scope,personaId:selected.personaId}:null});
    }
    if(action==='activate-scope' && req.method==='POST'){
      const label=String(body.syntheticScope||'').toUpperCase();
      if(!['A','B'].includes(label))return bad(res,'A 또는 B 검토 범위를 선택해주세요.');
      const info=await activateScope(client,res,label);
      return await scopeStore.run({personaId:info.personaId,planId:info.planId,scope:info.scope},async()=>json(res,200,{selected:info.scope,scope:{label:info.scope,personaId:info.personaId},snapshot:await snapshot(client)}));
    }
    const selected=await selectedScope(client,req);
    if(!selected)return json(res,428,{error:'검토 범위를 먼저 선택해주세요.',code:'SCOPE_REQUIRED'});
    return await scopeStore.run({personaId:selected.personaId,planId:selected.planId,scope:selected.scope}, async()=>{
      if(action==='reset-scope' && req.method==='POST'){await resetSelectedScope(client);return json(res,200,{selected:selected.scope,snapshot:await snapshot(client)});}

    if (action === 'bootstrap' && req.method === 'GET') return json(res, 200, {...await snapshot(client),reviewScope:selected.scope});
    ...
    if (action === 'complete-todo' && req.method === 'POST') {
      ...
      if(ins.rowCount===1)await client.query(`UPDATE todos SET status='done',updated_at=NOW() WHERE todo_id=$1 AND plan_id=$2`,[body.todoId,currentPlan()]);
